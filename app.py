...
def infer_mic_allowed_for_rooms(rooms_used: Set[str], gallery_678: bool) -> Tuple[bool, str]:
    ...
    return False, "マイク対象部屋（大会議室/小会議室 または 第6〜8条件）が含まれていません"
...
def render_kpis(room_total: int, equipment_total: int, tech_total: int, internet_total: int):
    ...
    c1.metric("部屋代 合計", yen(room_total))
    c2.metric("設備 合計", yen(equipment_total))
    c3.metric("技術者 合計", yen(tech_total))
    c4.metric("インターネット 合計", yen(internet_total))
    ...
    with right:
        ...
        if rooms_by_day:
            ...
            if _has_any(sel_map.get(PA_C_ID, 0)):
                ...
                if not eligible:
                    st.info("拡声装置C：対象日（大会議室/小会議室）がないため計算されません。")
            ...
            if _has_any(sel_map.get(MIC_WIRED_ID, 0)) or _has_any(sel_map.get(MIC_WIRELESS_ID, 0)):
                ...
                for d, rs in rooms_by_day.items():
                    ...
                    if ok:
                        eligible.append(d)
                if not eligible:
                    st.info("マイク：対象日がないため計算されません（大会議室/小会議室 または 第6〜8条件が必要です）。")
