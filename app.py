...
“”“日付→割増利用フラグのマップを返す。同一日付が複数行ある場合は最後の値が採用される。”””
m = {}
...
for d in active_dates:
    ...
    # base_selections はコピーして渡す（calc_equipment_total_for_day 内でも
    # 再度コピーするため二重保護だが、呼び出し元の安全を保証）
...
return []
mask = days_df[“休館日”] == True  # noqa: E712
...
with left:
    ...
    # 休館日の表示は warning（黄色）に変更 — 計算は正常に除外されるため
    if not edited_days.empty and bool(edited_days["休館日"].any()):
        closed_list = edited_days.loc[edited_days["休館日"] == True, "日付"].astype(str).tolist()  # noqa: E712
        closed_list = [d for d in closed_list if d]
        msg = " / ".join(closed_list[:20])
        suffix = "…" if len(closed_list) > 20 else ""
        st.warning(f"休館日が含まれています（計算から自動で除外されます）：{msg}{suffix}")
...
with right:
    ...
    if has_d_days_raw:
        ...
        gallery_678 = False
        # 条件外になったら session_state もリセット（部屋変更時の状態残留を防止）
