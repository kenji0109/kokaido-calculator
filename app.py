...
# ✅ここが重要：小集会室でマイクが0円になっていた原因は「小会議室」になってたこと
MIC_C_ROOMS = {"大会議室", "小集会室"}  # 拡声装置C（正）
...
# ✅ yen() 未定義エラー対策（今回のクラッシュ原因）
def yen(x) -> str:
    """金額をコンパクトに表示（スマホで…になりにくい）"""
    if x is None:
        return "—"
    try:
        v = int(float(x))
...
# 部屋名抽出（備考/品目名から推定用）
_ROOM_NAME_PAT = re.compile(
    r"(大会議室|大集会室|中集会室|小集会室|特別室|控え室\s*1|控え室\s*2|控室\s*1|控室\s*2|第\s*\d+\s*会議室)"
)
...
def parse_rooms_cell(cell: str) -> List[str]:
    ...
    applies_to_rooms 用：
    - 空/「*」/all → ["*"]
    - 句読点/中点/スラッシュ/空白/全角空白/パイプ → 区切り扱い
    - カッコは除去（中身は残す）
    - 第6〜8会議室 のレンジ展開
    """
    ...
    # ✅ groupのCSVが「大会議室|第1会議室|...」だったので '|' を区切りに追加
    for sep in ["|", "｜", "、", "，", ",", "・", "/", "／", ";", "；", "\n", "\t", "　", " "]:
        s = s.replace(sep, ",")
...
def infer_item_target_rooms(item_name: str, notes: str, fallback: str) -> str:
    """
    品目名/備考から対象部屋を推定して表示に使う。
    推定できない場合は fallback（通常はグループの applies_to_rooms）を返す。
    """
...
def calc_equipment_total_for_day(
    ...
    - マイク/拡声装置は「日別の部屋構成」で課金可否を決定（対象外日は課金0、明細は残す）
    """
    ...
    for s in selections:
        ...
        # billed_qty=0でも、マイク関連は“選んでいた事実”を残す
...
def calc_equipment_total_all_days(
    ...
    ※ マイク/拡声装置は selections 内に含め、日別判定で対象日のみ課金
    """
...
# =========================
# Sticky Total UI（スマホ最強）
# =========================
def inject_totals_css():
    st.markdown(
        """
<style>
/* 画面上部に固定する領域 */
...
.oai-breakdown span{ white-space: nowrap; }

@media (max-width: 640px){
  .oai-total-amount{ font-size: 28px; }
}
</style>
        """,
...
def render_totals_sticky(room_total: int, equipment_total: int, tech_total: int, internet_total: int):
    ...
    st.markdown(
        f"""
<div class="oai-sticky-wrap">
  <div class="oai-total-card">
    <div class="oai-total-amount">{yen(grand_total)}</div>
    <div class="oai-breakdown">
      <span>部屋 {yen(room_total)}</span>
      <span>設備 {yen(equipment_total)}</span>
      <span>技術者 {yen(tech_total)}</span>
      <span>ネット {yen(internet_total)}</span>
    </div>
    <div class="oai-total-sub">※直近の計算結果です。条件を変更したら「計算する」を押してください。</div>
  </div>
</div>
        """,
        unsafe_allow_html=True,
...
def render_kpis(room_total: int, equipment_total: int, tech_total: int, internet_total: int):
    ...
    c1.markdown(f"**部屋代 合計**\n\n### {yen(room_total)}")
    c2.markdown(f"**設備 合計**\n\n### {yen(equipment_total)}")
    c3.markdown(f"**技術者 合計**\n\n### {yen(tech_total)}")
    c4.markdown(f"**インターネット 合計**\n\n### {yen(internet_total)}")
    c5.markdown(f"**総額**\n\n### {yen(grand_total)}")
...
def main():
    ...
    # ✅ 直近の計算結果があれば、常に上部に sticky 表示（スマホで超便利）
    last = st.session_state.get("last_calc", None)
    if isinstance(last, dict) and all(k in last for k in ["room_total", "equipment_total", "tech_total", "internet_total"]):
        render_totals_sticky(
            int(last["room_total"]),
            int(last["equipment_total"]),
            int(last["tech_total"]),
            int(last["internet_total"]),
        )
    ...
    with left:
        ...
        # ✅ 休館日があるなら赤文字で明確に警告（ビジネス用）
        if not edited_days.empty and bool(edited_days["休館日"].any()):
            closed_list = edited_days.loc[edited_days["休館日"] == True, "日付"].tolist()
            closed_str = " / ".join(closed_list[:8]) + (" ..." if len(closed_list) > 8 else "")
            st.markdown(f"**:red[休館日があります：{closed_str}]**")
